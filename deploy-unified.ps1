...
#
# Usage:
#   .\deploy-unified.ps1              # Deploy to both platforms
#   .\deploy-unified.ps1 -Platform vercel    # Deploy to Vercel only
#   .\deploy-unified.ps1 -Platform render    # Deploy to Render only
#   .\deploy-unified.ps1 -Help               # Show help
# ============================================
...
function Show-Help {
    ...
    Write-Host "    .\deploy-unified.ps1 [-Platform <platform>] [-Help]"
    Write-Host ""
    ...
    Write-Host "    .\deploy-unified.ps1                      # Deploy to both platforms"
    Write-Host "    .\deploy-unified.ps1 -Platform vercel     # Deploy to Vercel only"
    Write-Host "    .\deploy-unified.ps1 -Platform render     # Deploy to Render only"
    Write-Host ""
    Write-Host "ENVIRONMENT VARIABLES (set in .env or platform dashboard):" -ForegroundColor Yellow
    Write-Host "    Required:"
    Write-Host "    - JWT_SECRET              Secure random string (32+ chars)"
    Write-Host "    - NODE_ENV                production"
    Write-Host ""
    Write-Host "    Optional:"
    Write-Host "    - PRINTFUL_API_KEY        Printful integration"
    Write-Host "    - CANVA_API_KEY           Canva automation"
    Write-Host "    - STRIPE_API_KEY          Payment processing"
    Write-Host "    - OPENAI_API_KEY          AI features"
    Write-Host "    - SENTRY_DSN              Error monitoring"
    Write-Host ""
    Write-Host "    Automation Settings:"
    Write-Host "    - MAX_PRODUCTS            Max products per run (default: 5)"
    Write-Host "    - GENERATE_DESIGNS        Enable design generation (default: true)"
    Write-Host "    - CREATE_LISTINGS         Enable listings (default: true)"
    Write-Host "    - GENERATE_MARKETING      Enable marketing (default: true)"
    Write-Host "    - GLOBAL_TRENDING         Enable trending discovery (default: true)"
    Write-Host "    - TRENDING_REGIONS        Regions to check (default: US,GB,CA,AU,DE,FR,JP,BR,IN,MX)"
    Write-Host ""
    Write-Host "MORE INFO:" -ForegroundColor Yellow
    Write-Host "    - Full Guide: UNIFIED-DEPLOYMENT-GUIDE.md"
    Write-Host "    - Quick Start: QUICK-START-UNIFIED.md"
    Write-Host "    - Vercel docs: https://vercel.com/docs"
    Write-Host "    - Render docs: https://render.com/docs"
    Write-Host ""
    exit 0
}
...
function Setup-Environment {
    ...
    if (-not (Test-Path .env)) {
        ...
        if (Test-Path .env.example) {
            ...
            Write-Warning-Custom "⚠️  Please edit .env with your actual API keys before deploying!"
            Write-Host ""
